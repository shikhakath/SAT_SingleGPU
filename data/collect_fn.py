...
def collect_fn(data):
    ...
    elif isinstance(data[0], tuple): #CTDataset
        input_ids = torch.cat([item[0] for item in data], dim=0) #concatenate input ids
        attention_mask = torch.cat([item[1] for item in data], dim=0)  # Concatenate attention_mask
        if len(data[0]) == 3:  # If labels are present (infer=False)
            labels = torch.stack([item[2] for item in data], dim=0)  # Stack labels
            return {
                'input_ids': input_ids,
                'attention_mask': attention_mask,
                'labels': labels,
            }
        else:  # If inference mode (infer=True)
            return {
                'input_ids': input_ids,
                'attention_mask': attention_mask,
            }
    else:
        raise ValueError("Unsupported data format in collect_fn.")
